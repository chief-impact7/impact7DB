...
async function loadClassSettings() {
    try {
        ...
        showToast('수업 설정 로드 실패: ' + err.message, 'warn');
...
const _inflightLeaveActions = new Set(); // docId 기반 연타 방지
...
const ACTIVE_STUDENT_STATUSES = ENROLLABLE_STATUSES;
...
// HTML 이스케이프 — shared html-escape (5종, 순수 문자열 기반)
const esc = _sharedEsc;
const escAttr = _sharedEscAttr;
...
export const enrollmentCode = _enrollmentCode;
...
// 단지 파생 — shared branch.js
const branchFromClassNumber = _branchFromClassNumber;
...
async function loadLeaveRequests() {
    try {
        const snapshot = await getDocs(query(collection(db, 'leave_requests'), where('status', 'in', ['requested', 'teacher_approved'])));
        leaveRequests = [];
        ...
        // 캐시/서버 스냅샷에서 연속 호출되므로 같은 실패 토스트의 단시간 중복은 억제
        if (Date.now() - _lrFailToastAt > 5000) {
            _lrFailToastAt = Date.now();
            showToast('휴퇴원 요청 로드 실패: ' + error.message, 'error');
        }
    }
}
let _lrFailToastAt = 0;
...
window.handleLevelSemesterChange = (level, semester) => {
    ...
    storeUpdate({ activeFilters: { ...activeFilters } });
    updateReadonlyBanner();
    applyFilterAndRender();
};
...
function getCurrentSemester(level) {
    ...
    storeUpdate({ currentSemesterByLevel: { ...currentSemesterByLevel } });
...
// ---------------------------------------------------------------------------
// docId generator (upsert-students.js와 동일한 방식)
// ---------------------------------------------------------------------------
...
window.submitNewStudent = async () => {
    ...
    try {
        ...
        try {
            ...
            showToast('저장됨, 화면 갱신 실패 — 새로고침 하세요.', 'warn');
        }
    } catch (err) {
...
function _renderEnrollmentAccountCard(container, account, enrollments) {
    ...
    card.innerHTML = `
        <div class="enrollment-card-header">
            ...
            ${accountItemIdx >= 0 ? `<button class="btn-end-class" onclick="window.endEnrollment(${accountItemIdx})" title="계정 종료" aria-label="계정 종료">${msIcon('sign-out')}</button>` : ''}
        </div>
...
window.saveEnrollment = async () => {
    ...
    // context 없는 진입 경로 없음 (openEnrollmentModal 폐기) — form/edit 분기에서 반드시 반환됨.
};
...
window.confirmEndClassSingle = async () => {
    ...
    const allBtn = document.getElementById('end-class-confirm-btn');
    ...
    try {
        ...
        const aBtn = document.getElementById('end-class-confirm-btn');
...
window.handleSheetUrlUpload = async () => {
    const url = await promptModal({
        title: '시트 URL로 업로드',
        label: '구글시트 URL',
        placeholder: 'https://docs.google.com/spreadsheets/d/...',
        hint: '구글시트가 열린 상태에서 주소창의 URL을 복사해 붙여넣으세요.',
    });
    if (!url) return;
    ...
    try {
        ...
        if (tabs.length > 1) {
            const choice = await promptModal({
                title: '가져올 탭 선택',
                label: `"${sheetName}"에 탭이 ${tabs.length}개 있습니다.`,
                options: tabs,
            });
            if (choice === null) return;
            const idx = Number(choice);
            if (!Number.isInteger(idx) || idx < 0 || idx >= tabs.length) { showToast('올바른 탭을 선택해주세요.', 'warn'); return; }
...
function formatTenure(start, end, startEvent, studentData) {
...
function _renderLeaveRequestRow(r, studentId, studentStatus = '') {
    ...
    let metaHtml = `<div style="font-size:10px;color:var(--text-sec);margin-top:4px;display:flex;gap:8px;flex-wrap:wrap;">
        ...
        ${appBy ? `<span>행정부: ${esc(appBy)} ${_fmtTs(r.approved_at)}</span>` : ''}
    </div>`;
...
window.submitReturnFromLeave = async () => {
    ...
    const rflOverlay = document.getElementById('return-from-leave-modal');
    const rflSubmitBtn = document.getElementById('rfl-submit-btn');
    rflOverlay.dataset.busy = '1';
    if (rflSubmitBtn) { rflSubmitBtn.disabled = true; rflSubmitBtn.textContent = '요청 중...'; }
    try {
        ...
        delete rflOverlay.dataset.busy;
        if (rflSubmitBtn) { rflSubmitBtn.disabled = false; rflSubmitBtn.textContent = '요청'; }
    }
};
...
// 내신 시간표 기능은 naesin-schedule.js로 분리됨
    ...
    try {
        ...
        for (let i = 0; i < toSave.length; i += BATCH_SIZE) {
            ...
            for (const entry of chunk) {
                ...
                // 재원계열이 아닌 학생(퇴원/종강/상담 + status 결손 레거시)에 특강을 붙이면 rules가
                // enrollment↔status 정합성으로 거부 → batch 전체 롤백. 일괄경로에서는 건너뛰고 안내.
                if (existingS && !ENROLLABLE_STATUSES.has(existingS.status)) {
                    skippedWithdrawn.push(`${entry.name}(${existingS.status || '상태없음'})`);
                    continue;
                }
...
// ─── 접근성: 키보드 활성화 (공유 a11y-dom.js) — 모달 Esc/Tab 트랩은 modal-manager.js가 담당 ──

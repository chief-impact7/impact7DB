...
description: "impact7 에코시스템(DB/DSC/HR/exam/tablet) 통합 운영 오케스트레이터. 기능 개발, 버그 수정, 데이터 모델 변경, 리팩토링 등 5개 앱에 걸친 모든 개발 작업을 조율한다. '기능 추가', '버그 수정', '필드 변경', '컬렉션 추가', '마이그레이션', '크로스앱 변경', '태블릿/키오스크/출결/외출 체크인' 요청 시 사용. 후속 작업: 결과 수정, 부분 재실행, 업데이트, 보완, 다시 실행, 이전 결과 개선 요청 시에도 반드시 이 스킬 사용. 단순 질문이나 단일 앱 내 소규모 변경은 직접 처리 가능."
---

# impact7 Orchestrator

impact7 에코시스템(DB/DSC/HR/exam/tablet)의 에이전트를 조율하여 크로스앱 개발 작업을 안전하게 수행하는 통합 오케스트레이터.

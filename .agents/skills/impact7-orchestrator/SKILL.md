...
description: "impact7 에코시스템(DB/DSC/HR/exam/tablet/mobile) 통합 운영 오케스트레이터. 기능 개발, 버그 수정, 데이터 모델 변경, 리팩토링 등 6개 앱에 걸친 모든 개발 작업을 조율한다. '기능 추가', '버그 수정', '필드 변경', '컬렉션 추가', '마이그레이션', '크로스앱 변경', '태블릿/키오스크/출결/외출 체크인', '모바일 앱/학부모 앱/어머님 소통/자료 공유/학부모 공지·알림' 요청 시 사용. 후속 작업: 결과 수정, 부분 재실행, 업데이트, 보완, 다시 실행, 이전 결과 개선 요청 시에도 반드시 이 스킬 사용. 단순 질문이나 단일 앱 내 소규모 변경은 직접 처리 가능."
---

# impact7 Orchestrator

impact7 에코시스템(DB/DSC/HR/exam/tablet/mobile)의 에이전트를 조율하여 크로스앱 개발 작업을 안전하게 수행하는 통합 오케스트레이터.
...
| db-developer | db-developer | impact7DB(app.js 프론트) 구현 | DB 프론트 영향 시 |
| dsc-developer | dsc-developer | impact7newDSC 구현 | DSC 영향 시 |
| tablet-developer | tablet-developer | impact7tablet(출결 키오스크 프론트) 구현 | 태블릿 프론트(키패드·액션·완료 흐름) 영향 시 |
| mobile-developer | mobile-developer | Mobile(학부모 소통·자료공유 앱 프론트) 구현 | Mobile 프론트 영향 시 |
| functions-developer | functions-developer | functions-shared(Cloud Functions 백엔드) 구현 | callable/scheduled/trigger·Gemini·secret·배포 영향 시 |
...
- **Cloud Functions(functions-shared) 영향 시**: functions-developer 호출. 백엔드 콜러블과 프론트(DSC) 연동이 함께면 functions-developer + dsc-developer를 병렬 호출하고, 콜러블 이름·반환 shape 정합성을 qa-validator로 검증한다.
- **AI 콜러블 시그니처 변경·삭제 시**: functions-developer의 무중단 배포 순서(새 함수 배포 → 프론트 배포 → 구 함수 삭제)를 따른다.
- **Mobile 학부모 기능 시**: 백엔드 계약이 먼저다 — functions-developer(콜러블) → mobile-developer(프론트) 순서로 진행하고, 콜러블 반환 shape ↔ Mobile 클라 사용의 경계면을 qa-validator로 검증한다. 학부모 인증 모델 확정 전에는 rules 완화 금지(callable 경유 원칙).
- **태블릿 출결/외출 작업 시**: 백엔드 계약이 먼저다. functions-developer(`tabletCheckin` callable·상태머신·알림톡 enqueue) → tablet-developer(키오스크 프론트, 서버 `allowedActions` 신뢰) 순서로 진행하고, 하원 게이트가 걸리면 dsc-developer(`daily_records.checklist_complete` 캐시)도 함께 호출한다. callable 반환 shape ↔ tablet 클라 사용의 경계면을 qa-validator로 검증한다. 태블릿은 Firestore 직접 접근이 없으므로(callable 경유) rules 변경은 신규 컬렉션(`attendance_events`/`kiosk_devices`) 추가 시에만 필요하다.

**학생 AI 리포트 도메인:** 학생 종합 리포트·종합상태 카드·상담 요약/브리핑·Chat 연동·월말 리포트 작업이면, functions-developer/dsc-developer 프롬프트에 `student-report-ai` 스킬을 참조하도록 지시한다(데이터 모델 함정·통합 핸들러 패턴·배포 순서가 정리돼 있음).
...
### 정상 흐름: 태블릿 출결 기능 (크로스앱)
1. 사용자: "태블릿에서 외출/복귀도 처리되게 해줘"
2. Phase 1: impact-analyst가 functions-shared(`tabletCheckin` 상태머신) + tablet(프론트 버튼) + DSC(체크리스트 캐시) 영향 파악
3. Phase 2: functions-developer가 백엔드 callable·상태머신 구현 → tablet-developer가 `allowedActions` 기반 버튼 렌더 → (하원 게이트 시) dsc-developer가 캐시 기록
4. Phase 3: qa-validator가 callable 반환 shape ↔ tablet 클라 사용 경계면 검증
5. Phase 4: 빌드 확인, 결과 보고

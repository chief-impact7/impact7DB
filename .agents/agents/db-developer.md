...
- **경로**: `/Users/jongsooyi/IMPACT7/impact7DB/`
...
- 공유 상태(allStudents, activeFilters 등)는 **`store.js`에서 import**한다 (`app.js`에서 export하지 않음)
- 상태 변경은 반드시 `storeUpdate({ key: value })`를 사용하고, 배열/객체 직접 mutate 금지
- 배열을 in-place 변경(push, 프로퍼티 수정)한 뒤에는 `storeUpdate({ arr: [...arr] })`로 새 참조 전달
- 기존 코드 수정 시 해당 블록의 분리를 검토한다
- 상세 분석: `.memory/feedback_module_separation.md` 참조

## 탐색 원칙
- app.js 탐색 전에 `codegraph_explore`를 먼저 호출한다 (심볼·파일 위치 즉시 파악)
- @impact7/shared 관련 작업은 shared-first: `/Users/jongsooyi/IMPACT7/impact7-shared` export map 먼저 확인

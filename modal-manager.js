// 모달 공통 매니저 — .modal-overlay의 display 변경을 감지해
...
// 1. 정적 마크업 또는 body 직속 동적 추가로 존재하는 .modal-overlay여야 한다
...
const observer = new MutationObserver((mutations) => {
    for (const m of mutations) syncOverlay(m.target);
});
    ...
    for (const m of mutations) {
        for (const node of m.addedNodes) {
            if (node.nodeType === 1 && node.classList?.contains('modal-overlay')) {
                ...
                // 이 콜백은 microtask라 append 직후 같은 태스크에서 이미 열렸을 수 있음 — 상태 동기화
                syncOverlay(node);
            }
        }
    }

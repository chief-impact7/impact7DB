...
- **모델 (2026-05-22 최종):** 텍스트·모든 폴백·게이트웨이 기본 = **gemini-3.5-flash**. 비전/커멘터리/분석 PRIMARY = **gemini-3.1-pro-preview**. DSC = gemini-3.5-flash. 게이트웨이 ALLOWED=[3.5-flash, 3.1-pro-preview]. (`gemini-3.1-pro` GA는 아직 Vertex 404 — preview 사용 중)
- **⚠️ SDK 교훈 — 에코시스템 전체 `@google/genai` 사용 (절대 `@google-cloud/vertexai` 쓰지 말 것):** exam을 처음 `@google-cloud/vertexai`로 전환했으나 **App Hosting 런타임에서 generate가 HTML 500 크래시**(프로세스 레벨, route의 JSON 에러핸들링 도달 못 함). 모델 무관(2.5-pro로도 동일), 빈/정상 데이터 모두 재현. 원인 = **deprecated `@google-cloud/vertexai`(2026-06-24 제거)가 App Hosting Cloud Run에서 비호환**. **`@google/genai`(vertex 모드)로 재전환하니 3.1-pro-preview generate 200 + 실제 분석 생성 확인(~30초, thinking 모델).** DSC도 게이트웨이(@google/genai)라 에코시스템 SDK = `@google/genai`로 통일. 빌드 SUCCESS ≠ 런타임 동작 — Vertex 변경은 반드시 실제 호출로 검증할 것.

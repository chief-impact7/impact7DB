...
- **모델 (2026-05-22):** 텍스트·모든 폴백·게이트웨이 기본 = **gemini-3.5-flash**. 비전/커멘터리/분석 PRIMARY = **gemini-2.5-pro**. DSC = gemini-3.5-flash. 게이트웨이 ALLOWED=[3.5-flash, 2.5-pro].
- **⚠️ 3.1-pro-preview 롤백 (2026-05-22):** exam pro를 잠깐 gemini-3.1-pro-preview로 올렸으나 **generate가 500(HTML 응답, 즉시 실패)** — route는 try/catch+JSON 에러인데 HTML이 온 건 프로세스 레벨 크래시. **deprecated `@google-cloud/vertexai` SDK가 3.1-pro-preview(thinking 신모델, thoughtSignature 응답)와 비호환 추정** (REST/curl은 200, SDK 레벨에서 깨짐). 정상 데이터(문항 25개)로도 재현. **2.5-pro로 롤백.** 3.1-pro-preview 재도입은 exam을 `@google/genai`(권장 SDK)로 전환한 뒤 재시도할 것. (`gemini-3.1-pro` GA는 아직 Vertex 404)

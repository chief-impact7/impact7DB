...
- 학부: `초등` · `중등` · `고등` · `졸업` · `All`
- 학년: 학부에 따라 동적 (초 1–6, 중 1–3, 고 1–3, 졸업은 N/A)
...
openPromoExtractModal()
    ↓
모달 표시, 기본 필터(상태=재원, 학부=All)로 초기 렌더
    ↓
필터 변경 시마다:
    1. store.allStudents에서 상태 필터 1차 적용
    2. 각 학생에 normalizeRealLevelGrade(s) 호출
    3. 정규화 결과로 학부/학년 필터 2차 적용
...
- 학부 단일선택 변경 시 학년 체크박스 동적 재구성 (초=1~6, 중/고=1~3, 졸업=숨김)
- 졸업 선택 시 학년 필터 비활성(전체 표시)
...
- [ ] 재원 + 초등 + 학년 3,4 필터 → 정확한 카운트 매칭
- [ ] 비원 + 학부 All → 가상승급 적용된 학년으로 표시
- [ ] 비원 + 졸업 → 가상승급 결과 고3 초과한 학생만 표시

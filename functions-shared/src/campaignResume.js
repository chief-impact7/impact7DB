...
export const REQUEST_FINGERPRINT_VERSION = 2;
...
export function campaignFingerprintMatches(value, fingerprint, legacyFingerprints = []) {
  return [fingerprint, ...legacyFingerprints].includes(value ?? null);
}
...
export async function claimCampaignResume(db, campaignRef, {
  now,
  content,
  fingerprint,
  legacyFingerprints = [],
  ...
  return db.runTransaction(async (tx) => {
    ...
    const acceptedLegacy = ex.request_fingerprint_version === REQUEST_FINGERPRINT_VERSION
      ? []
      : legacyFingerprints;
    if (ex.content !== content || !campaignFingerprintMatches(ex.request_fingerprint, fingerprint, acceptedLegacy)) {
